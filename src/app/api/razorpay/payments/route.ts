...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (cache && cache.key === cacheKey && Date.now() - cache.ts < CACHE_TTL) {
      return NextResponse.json({ payments: cache.data, _permissions: result.permissions });
    }

    if (inflight && inflight.key === cacheKey) {
      const data = await inflight.promise;
      return NextResponse.json({ payments: data, _permissions: result.permissions });
    }
    ...
    inflight = null;
    return NextResponse.json({ payments: normalized, _permissions: result.permissions });
  } catch (error) {
    inflight = null;
