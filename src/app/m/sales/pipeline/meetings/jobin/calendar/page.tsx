...
export default function JobinCalendarPageWrapper() {
  return (
...
function JobinCalendarPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Select only calendars where Jobin is a team member
        const jobinCalendars = jobin
          ? cals.filter((c: CalendarItem) =>
              c.teamMembers?.some((tm: { userId: string }) => tm.userId === jobin.id)
            )
          : [];
        setSelectedCalendars(
          new Set(
            jobinCalendars.length > 0
              ? jobinCalendars.map((c: CalendarItem) => c.id)
  ...
  const { rangeStart, rangeEnd, monthGrid } = useMemo(() => {
    ...
    if (view === "day") {
      ...
      const wDates = getWeekDates(baseDate);
  ...
  /* ── Filtered events (Jobin only) ────────────── */
  const filteredEvents = useMemo(() => {
    return events.filter((ev) => {
      if (!selectedCalendars.has(ev.calendarId)) return false;
      if (jobinUserId && ev.assignedUserId !== jobinUserId) return false;
      return true;
    });
  }, [events, selectedCalendars, jobinUserId]);
  ...
  const userNameMap = useMemo(() => {
    const map: Record<string, string> = {};
    if (jobinUserId) map[jobinUserId] = "Jobin";
    return map;
  }, [jobinUserId]);
