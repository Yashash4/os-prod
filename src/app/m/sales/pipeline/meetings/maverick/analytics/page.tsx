  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const mav = usrs.find((u: GHLUser) => u.name.toLowerCase().includes("maverick"));
        const mavId = mav?.id || null;
        ...
        if (mav && calData.calendars) {
          const cals = calData.calendars as CalendarItem[];
          const mavCals = cals.filter((c) => c.teamMembers?.some((tm) => tm.userId === mav.id));
          if (mavCals.length > 0) {
            const now = new Date();
            ...
            setCalendarEvents(allEvents.filter((ev) => ev.assignedUserId === mav.id));
          }
        }
