...
function MaverickCalendarPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Find Maverick's user ID
        const mav = usrs.find((u: GHLUser) =>
          u.name.toLowerCase().includes("maverick")
        );
        if (mav) setMaverickUserId(mav.id);

        // Select only calendars where Maverick is a team member
        const mavCalendars = mav
          ? cals.filter((c: CalendarItem) =>
              c.teamMembers?.some((tm: { userId: string }) => tm.userId === mav.id)
            )
          : [];
  ...
  /* ── Filtered events (Maverick only) ────────────── */
  const filteredEvents = useMemo(() => {
    return events.filter((ev) => {
      if (!selectedCalendars.has(ev.calendarId)) return false;
      if (maverickUserId && ev.assignedUserId !== maverickUserId) return false;
      return true;
    });
  }, [events, selectedCalendars, maverickUserId]);
  ...
  const userNameMap = useMemo(() => {
    const map: Record<string, string> = {};
    if (maverickUserId) map[maverickUserId] = "Maverick";
    return map;
  }, [maverickUserId]);
