...
export default function MeetManagementPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        if (maverick) setMaverickUserId(maverick.id);
        setUsersLoaded(true);
        ...
        if (maverick && calData.calendars) {
          ...
          if (mavCals.length > 0) {
            ...
            allEvents
              .filter((ev) => ev.assignedUserId === maverick.id && ev.contactId)
  ...
  // Filter records to Maverick's leads only
  const maverickRecords = useMemo(() => {
    if (!usersLoaded) return [];
    if (!maverickUserId) return records;
    return records.filter((r) => r.assigned_to === maverickUserId);
  }, [records, maverickUserId, usersLoaded]);
  ...
  const filteredRecords = useMemo(() => {
    return maverickRecords.filter((r) => {
  ...
  }, [maverickRecords, statusFilter, searchQuery, dateFilter, eventsByContact]);

  const statusCounts = useMemo(() => {
    return ALL_GHL_STATUSES.reduce((a, s) => {
      a[s] = maverickRecords.filter((r) => (r.ghl_status || "open") === s).length;
      return a;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-3 flex items-center gap-3 flex-shrink-0 border-b border-border/50 bg-surface/50">
          ...
          {showFilterDropdown && (
            <div className="absolute top-full left-0 mt-1 w-48 bg-surface border border-border rounded-lg shadow-xl z-20 py-1">
              <button onClick={() => { setStatusFilter("all"); setShowFilterDropdown(false); }} className={`w-full text-left px-3 py-1.5 text-sm transition-colors ${statusFilter === "all" ? "text-accent bg-accent/5" : "text-foreground hover:bg-surface-hover"}`}>
                All Statuses ({maverickRecords.length})
              </button>
        ...
        <span className="text-xs text-muted ml-auto">Showing {filteredRecords.length} of {maverickRecords.length}</span>
      </div>
            ...
            {filteredRecords.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS.length} className="text-center py-16 text-muted text-sm">
                  {maverickRecords.length === 0
                    ? 'No leads assigned to Maverick yet. Make sure Call Booked data is synced in Sales Setting.'
              ...
              <tr key={record.opportunity_id} className="border-b border-border/50 hover:bg-surface-hover/50 transition-colors group">
                ...
                <td className="px-2 py-1.5 border-r border-border">
                  <PermissionGate module="sales" subModule="sales-maverick-meet" action="canApprove" fallback={<span className="text-[11px]">{GHL_STATUS_CONFIG[record.ghl_status || "open"]?.label || record.ghl_status || "Open"}</span>}>
                    <GHLStatusDropdown
                ...
                <td className="px-3 py-2 text-xs border-r border-border cursor-pointer" onClick={() => startEdit(record.opportunity_id, "meet_notes", record.meet_notes || "")}>
                  <PermissionGate module="sales" subModule="sales-maverick-meet" action="canEdit" fallback={
                ...
                <td className="px-3 py-2 border-r border-border">
                  {(() => {
                    ...
                    return (
                      <Link
                        href={`/m/sales/pipeline/meetings/maverick/calendar?eventId=${ev.id}&tab=form`}
