...
export default function SalesManagementPage() {
  ...
  // Filter to Maverick's won leads only
  ...
  }, [maverickRecords, collectionFilter, searchQuery]);

  // Summary stats
  const stats = useMemo(() => {
    const totalQuoted = maverickRecords.reduce((s, r) => s + (r.fees_quoted || 0), 0);
    const totalCollected = maverickRecords.reduce((s, r) => s + (r.fees_collected || 0), 0);
    const totalPending = maverickRecords.reduce((s, r) => s + (r.pending_amount || 0), 0);
    const fullyPaid = maverickRecords.filter((r) => r.collection_status === "fully_paid").length;
    return { totalQuoted, totalCollected, totalPending, fullyPaid, total: maverickRecords.length };
  }, [maverickRecords]);
  ...
  const updateSalesRecord = async (oppId: string, updates: Record<string, unknown>) => {
    try {
      ...
      const res = await apiFetch("/api/sales/maverick-sales-tracking", {
        method: "PUT",
