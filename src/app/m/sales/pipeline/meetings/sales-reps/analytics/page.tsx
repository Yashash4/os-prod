...
function AnalyticsPageInner() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const repUser = usrs.find((u: GHLUser) => u.name.toLowerCase().includes(repLabel.toLowerCase()));
        
        setRepGhlUserId(repUser?.id || null);
        ...
        if (repUser && calData.calendars) {
          ...
          const mavCals = cals.filter((c) => c.teamMembers?.some((tm) => tm.userId === repUser.id));
          if (mavCals.length > 0) {
            const now = new Date();
            ...
            setCalendarEvents(allEvents.filter((ev) => ev.assignedUserId === repUser.id));
          }
        }
  ...
  // Filter to this rep's records (by GHL user ID)
  const mavMeetRecords = useMemo(() => {
    if (!repGhlUserId) return meetRecords;
    return meetRecords.filter((r) => r.assigned_to === repGhlUserId);
  }, [meetRecords, repGhlUserId]);
  const mavSalesRecords = useMemo(() => {
    if (!repGhlUserId) return salesRecords;
    return salesRecords.filter((r) => r.assigned_to === repGhlUserId);
  }, [salesRecords, repGhlUserId]);

  // Filtered records based on global filters
  const filteredMeetRecords = useMemo(() => {
    return mavMeetRecords.filter((r) => {
      if (ghlFilter !== "all" && (r.ghl_status || "open") !== ghlFilter) return false;
      if (setterFilter !== "all" && r.status !== setterFilter) return false;
      return true;
    });
  }, [mavMeetRecords, ghlFilter, setterFilter]);
  ...
  const filteredSalesRecords = useMemo(() => {
    if (ghlFilter === "all" && setterFilter === "all") return mavSalesRecords;
    return mavSalesRecords.filter((r) => filteredOppIds.has(r.opportunity_id));
  }, [mavSalesRecords, filteredOppIds, ghlFilter, setterFilter]);
  ...
  const ghlCounts = useMemo(() => {
    const counts: Record<string, number> = { all: mavMeetRecords.length, open: 0, won: 0, lost: 0, abandoned: 0 };
  ...
  return (
    <div className="flex flex-col h-full overflow-auto">
      ...
      <div className="px-6 py-3 border-b border-border flex-shrink-0 space-y-2">
        ...
        {(ghlFilter !== "all" || setterFilter !== "all") && (
          <p className="text-[11px] text-muted">Showing {filteredMeetRecords.length} of {mavMeetRecords.length} leads</p>
        )}
      </div>
...
export default function AnalyticsPage() {
  return <Suspense fallback={<div className="flex justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#B8860B] border-t-transparent" /></div>}><AnalyticsPageInner /></Suspense>;
}
