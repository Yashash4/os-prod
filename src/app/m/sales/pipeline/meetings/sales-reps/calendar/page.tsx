...
export default function RepCalendarPageWrapper() {
  return (
...
function RepCalendarPage() {
  const searchParams = useSearchParams();
  const repId = searchParams.get("repId");
  const repLabel = searchParams.get("repLabel") || "Sales Rep";
  const autoEventId = searchParams.get("eventId");
  const autoTab = searchParams.get("tab") as DetailTab | null;
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Select only calendars where Rep is a team member
        const mavCalendars = repUser
          ? cals.filter((c: CalendarItem) =>
              c.teamMembers?.some((tm: { userId: string }) => tm.userId === repUser.id)
            )
          : [];
        setSelectedCalendars(
          new Set(
            mavCalendars.length > 0
              ? mavCalendars.map((c: CalendarItem) => c.id)
              : [cals[0]?.id].filter(Boolean)
          )
        );
      } catch (err) {
  ...
  /* ── Auto-open event from query params ────────────── */
  useEffect(() => {
    if (!autoEventId || autoOpenHandled.current || loading) return;

    // Try from loaded events first
    const ev = events.find((e) => e.id === autoEventId);
    if (ev) {
      setSelectedEvent(ev);
      if (autoTab) setDetailTab(autoTab);
      setShowSidebar(false);
      autoOpenHandled.current = true;
      // Navigate calendar to event date
      setBaseDate(new Date(ev.startTime));
      return;
    }

    // If events loaded but event not found, it might be in a different month
    // Fetch the event directly via calendar-events API using a wide range
    if (!eventsLoading && calendars.length > 0 && !autoOpenHandled.current) {
      autoOpenHandled.current = true;
      const now = new Date();
      ...
      Promise.all(
        ...
        for (const data of results) {
          const found = (data.events || []).find(
            (e: CalendarEvent) => e.id === autoEventId
          );
          if (found) {
            setSelectedEvent(found);
            if (autoTab) setDetailTab(autoTab);
            setShowSidebar(false);
            setBaseDate(new Date(found.startTime));
            break;
          }
        }
      });
    }
  }, [autoEventId, autoTab, events, loading, eventsLoading, calendars, selectedCalendars]);
  ...
  /* ── Filtered events (Rep only) ────────────── */
  const filteredEvents = useMemo(() => {
    return events.filter((ev) => {
      if (!selectedCalendars.has(ev.calendarId)) return false;
      if (repGhlUserId && ev.assignedUserId !== repGhlUserId) return false;
      return true;
    });
  }, [events, selectedCalendars, repGhlUserId]);
  ...
  const userNameMap = useMemo(() => {
    const map: Record<string, string> = {};
    if (repGhlUserId) map[repGhlUserId] = repLabel;
    return map;
  }, [repGhlUserId]);
  ...
  function getEventDisplayName(ev: CalendarEvent) {
    ...
    // Try to resolve name from our tracking data using contactId
      ...
      {showSidebar && !selectedEvent && (
        <div className="w-64 border-l border-border bg-surface flex-shrink-0 overflow-y-auto">
          <div className="p-3">
            ...
            {/* Filters header */}
            <div className="mb-2">
              <span className="text-xs font-medium text-muted">Calendars</span>
            </div>
