...
interface MergedRecord {
  ...
  stage_name: string;
  source: string;
  // Sales setter fields (read-only)
  status: string; // call_booked status from sales setter
  rating: number | null;
  ...
  // GHL native fields
  ghl_status: string | null; // open, won, lost, abandoned
  // Maverick fields (editable in our DB)
...
function MeetManagementPageInner() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        setUsers(usrs);
        const repUser = usrs.find((u: GHLUser) =>
          u.name.toLowerCase().includes(repLabel.toLowerCase())
        );
  ...
  // Filter records to this rep's leads only (by GHL user ID in assigned_to)
  const repRecords = useMemo(() => {
    if (!usersLoaded) return [];
    if (!repGhlUserId) return records; // Show all if rep not found in GHL
    return records.filter((r) => r.assigned_to === repGhlUserId);
  }, [records, repGhlUserId, usersLoaded]);

  // Update GHL opportunity status (pushes to GHL API)
  const updateGHLStatus = async (record: MergedRecord, newStatus: string) => {
    ...
    try {
      ...
      // Update local state + persist to our DB
      setRecords((prev) =>
        prev.map((r) =>
  ...
  const updateMeetRecord = async (oppId: string, updates: Record<string, unknown>) => {
    try {
      const res = await apiFetch(`/api/sales/rep-meetings?repId=${repId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  // Filtering by GHL status + date
  const filteredRecords = useMemo(() => {
    return repRecords.filter((r) => {
  ...
  }, [repRecords, statusFilter, searchQuery, dateFilter, eventsByContact]);

  const statusCounts = useMemo(() => {
    return ALL_GHL_STATUSES.reduce((a, s) => {
      a[s] = repRecords.filter((r) => (r.ghl_status || "open") === s).length;
      return a;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-3 flex items-center gap-3 flex-shrink-0 border-b border-border/50 bg-surface/50">
          ...
          {showFilterDropdown && (
            <div className="absolute top-full left-0 mt-1 w-48 bg-surface border border-border rounded-lg shadow-xl z-20 py-1">
              <button onClick={() => { setStatusFilter("all"); setShowFilterDropdown(false); }} className={`w-full text-left px-3 py-1.5 text-sm transition-colors ${statusFilter === "all" ? "text-accent bg-accent/5" : "text-foreground hover:bg-surface-hover"}`}>
                All Statuses ({repRecords.length})
              </button>
        ...
        <span className="text-xs text-muted ml-auto">Showing {filteredRecords.length} of {repRecords.length}</span>
      </div>
            ...
            {filteredRecords.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS.length} className="text-center py-16 text-muted text-sm">
                  {repRecords.length === 0
                    ? 'No leads assigned to this rep yet. Make sure Call Booked data is synced in Sales Setting.'
              ...
              <tr key={record.opportunity_id} className="border-b border-border/50 hover:bg-surface-hover/50 transition-colors group">
                ...
                {/* Meet Date (from calendar events) */}
                ...
                {/* GHL Status (editable — updates GHL directly) */}
                <td className="px-2 py-1.5 border-r border-border">
                  <PermissionGate module="sales" subModule="rep-meet-management" action="canApprove" fallback={<span className="text-[11px]">{GHL_STATUS_CONFIG[record.ghl_status || "open"]?.label || record.ghl_status || "Open"}</span>}>
                    <GHLStatusDropdown
                      current={record.ghl_status || "open"}
                      onChange={(s) => updateGHLStatus(record, s)}
                    />
                  </PermissionGate>
                </td>

                {/* Setter Status (read-only) */}
                <td className="px-2 py-1.5 border-r border-border">
                  <StatusBadge status={record.status} config={CB_STATUS_CONFIG} />
                </td>

                {/* Setter Rating (read-only) */}
                <td className="px-2 py-1.5 border-r border-border">
                  <ReadOnlyStars value={record.rating || 0} />
                </td>

                {/* Setter Notes (read-only) */}
                <td className="px-3 py-2 text-xs border-r border-border">
                  <span className={record.notes ? "text-foreground" : "text-muted/40 italic"}>{record.notes || "-"}</span>
                </td>

                {/* Setter Comments (read-only) */}
                <td className="px-3 py-2 text-xs border-r border-border">
                  <span className={record.comments ? "text-foreground" : "text-muted/40 italic"}>{record.comments || "-"}</span>
                </td>

                {/* Meet Notes (editable — stored in our DB) */}
                <td className="px-3 py-2 text-xs border-r border-border cursor-pointer" onClick={() => startEdit(record.opportunity_id, "meet_notes", record.meet_notes || "")}>
                  <PermissionGate module="sales" subModule="rep-meet-management" action="canEdit" fallback={
                ...
                {/* Outcome (editable — stored in our DB) */}
                ...
                {/* Form Details (link to Calendar tab) */}
                <td className="px-3 py-2 border-r border-border">
                  {(() => {
                    ...
                    return (
                      <Link
                        href={`/m/sales/pipeline/meetings/sales-reps/calendar?repId=${repId}&repLabel=${encodeURIComponent(repLabel)}&eventId=${ev.id}&tab=form`}
...
/* ── GHL Status Dropdown (editable — updates GHL) ────── */
...
export default function MeetManagementPage() {
  return <Suspense fallback={<div className="flex justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#B8860B] border-t-transparent" /></div>}><MeetManagementPageInner /></Suspense>;
}
