...
function SalesManagementPageInner() {
    ...
    async function init() {
      try {
        const [usersRes, recordsRes] = await Promise.all([
          ...
          apiFetch(`/api/sales/rep-deals?repId=${repId}`),
        ]);
  ...
  // Filter records to this rep's won leads only (by GHL user ID)
  const repRecords = useMemo(() => {
    if (!usersLoaded) return [];
    if (!repGhlUserId) return records;
  ...
  }, [repRecords, collectionFilter, searchQuery]);

  // Summary stats
  const stats = useMemo(() => {
    const totalQuoted = repRecords.reduce((s, r) => s + (r.fees_quoted || 0), 0);
    const totalCollected = repRecords.reduce((s, r) => s + (r.fees_collected || 0), 0);
    const totalPending = repRecords.reduce((s, r) => s + (r.pending_amount || 0), 0);
    const fullyPaid = repRecords.filter((r) => r.collection_status === "fully_paid").length;
    return { totalQuoted, totalCollected, totalPending, fullyPaid, total: repRecords.length };
  }, [repRecords]);
  ...
  const updateSalesRecord = async (oppId: string, updates: Record<string, unknown>) => {
    try {
      ...
      const res = await apiFetch(`/api/sales/rep-deals?repId=${repId}`, {
        method: "PUT",
    ...
    <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-muted text-xs mt-0.5">
                {repLabel}'s won deals — {repRecords.length} deals
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="px-6 py-2 flex gap-2 flex-wrap flex-shrink-0 border-b border-border/50">
        <button onClick={() => setCollectionFilter("all")}
          ...
          All <span>{repRecords.length}</span>
        </button>
        {COLLECTION_STATUSES.map((cs) => {
          const count = repRecords.filter((r) => r.collection_status === cs.value).length;
          return (
            ...
            {filteredRecords.length === 0 ? (
              ...
              <tr key={record.opportunity_id} className="border-b border-border/50 hover:bg-surface-hover/50 transition-colors">
                ...
                <td className="px-3 py-2 text-xs border-r border-border">
                  <PermissionGate module="sales" subModule="rep-sales-management" action="canEdit" fallback={
                ...
                <td className="px-2 py-1.5 border-r border-border">
                  <PermissionGate module="sales" subModule="rep-sales-management" action="canApprove" fallback={
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <PermissionGate module="sales" subModule="rep-sales-management" action="canEdit">
                      <button
      ...
      {invoiceRecord && (
        <InvoiceSendModal
          ...
          salesSource={repLabel.toLowerCase()}
          onSuccess={(data) => {
...
export default function SalesManagementPage() {
  return <Suspense fallback={<div className="flex justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#B8860B] border-t-transparent" /></div>}><SalesManagementPageInner /></Suspense>;
}
